...
public class GridFragment extends BaseFragment {
    ...
    protected void populate() {
        ...
        tvView = find(R.id.fragment_grid_circle);
        //region  显示
        adapter = new GenericAdapter<QuestionResult>(getContext(),R.layout.grid_item,results) {
            @Override
            public void populate(ViewHolder viewHolder, QuestionResult questionResult) {
                TextView tvLabel=viewHolder.getView(R.id.grid_item_tv);

                viewHolder.setTextView(R.id.grid_item_tv,getPosition(questionResult)+1+"");
                if (questionResult.isRight()){
                    tvLabel.setBackgroundResource(R.drawable.grid_1);
                }else {
                    tvLabel.setBackgroundResource(R.drawable.grid_2);
                }
